...
if($_POST[nameOfSchedule])
{
    ...
    for($i = 0; $i < sizeof($scheduleArray); $i++)
    {
        ...
        {
            $_SESSION['error'] .= "Schedule Name is Already In Use<br>";
        }
    }
	...
	while($continue) 
	{
		...
		if($result = mysqli_query($link, $query))
		{
			...
			if($row[0] == '')
			{
				...
				$continue = false;
            mysqli_query($link, $query); 
            $query = "CREATE TABLE ".$tableName." (id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,reg_date TIMESTAMP, numberOfPeriods INT, numberOfStudents INT)"; 
            mysqli_query($link, $query); 
            ...
            $query = "INSERT INTO ".$tableName." (numberOfPeriods) VALUES (1)";
                mysqli_query($link, $query);
...
<div id="loggedInContent">
    ...
    <input class="form-control scheduleInput" type="text" id="nameOfScheduleText">
    </fieldset>
    </td>    

	
<td>
  <button type="submit" class="btn btn-primary" id="nameOfScheduleSubmit">Submit</button></td>
        
        </tr>

       </table>
    <div id="scheduleArray"></div>
    <table id="scheduleDisplayTable">

        <tr><td><hr></td></tr>
        
        <tr><th>Edit Old Schedules</th></tr>
        <tr></tr>
...
function displayTables()
{
    ...
    $.ajax({
            type: "POST",
            url: "adminLogedInActions.php?action=displaySched",
            data: "schedPassword=" + $('#nameOfScheduleText').val(),
            success: function(result) {
                $("#scheduleArray").html(result);               
            }
        })
}
...
function openSchedule(aValue)
{
    $.ajax({
            type: "POST",
            url: "adminLogedInActions.php?action=openSched",
            data: "schedNum=" + aValue,
            success: function(result) {
                alert(result);               
            }
        })
    window.location = "redo/";
}
        
function deleteSchedule(aValue)
{
    $.ajax({
            type: "POST",
            url: "adminLogedInActions.php?action=closeSched",
            data: "schedNum=" + aValue,
            success: function(result) {
                alert(result);               
            }
...
$("#nameOfScheduleSubmit").click(function() {
    
            $.ajax({
            type: "POST",
            url: "adminLogedInActions.php?action=makeSchedule",
            data: "schedName=" + $('#nameOfScheduleText').val(),
            success: function(result) {
                alert(result);                
            }
        })
